...
def zones_for_player(p):
    # Directionally plausible shot map from role + production when true zone tracking is unavailable.
    pid,name,team,age,height,weight,pos,jersey,ppg,apg,rpg,usg,ts,mom,*rest = p
    arch = compute_archetype(ppg,apg,rpg)
    elite = 5 if ppg >= 28 else 3 if ppg >= 22 else 1 if ppg >= 16 else 0
    high_usage = 2 if usg >= 30 else 1 if usg >= 25 else 0
    paint = LEAGUE_ZONE_AVG["Paint"] + elite + high_usage + (7 if arch=="Rim Anchor" else 4 if arch=="All-Around Big" else 3 if arch in ["Elite Scorer","Two-Way Engine"] else 1)
    ft = LEAGUE_ZONE_AVG["Free Throw"] + elite + high_usage + (4 if arch=="Elite Scorer" else 2 if arch=="Playmaker" else 1)
    mid_l = LEAGUE_ZONE_AVG["Left Midrange"] + elite + (4 if arch=="Elite Scorer" else 2 if arch in ["Playmaker","Two-Way Engine"] else 0)
    mid_r = mid_l + mom*.12
    corner_l = LEAGUE_ZONE_AVG["Left Corner 3"] + (3 if arch in ["Scoring Wing","Two-Way Engine"] else 1 if arch=="Playmaker" else -4 if arch=="Rim Anchor" else 0) + max(0,ts-57)*.15
    corner_r = LEAGUE_ZONE_AVG["Right Corner 3"] + (3 if arch in ["Scoring Wing","Two-Way Engine"] else 1 if arch=="Playmaker" else -4 if arch=="Rim Anchor" else 0) + max(0,ts-57)*.12
    wing_l = LEAGUE_ZONE_AVG["Left Wing 3"] + (4 if arch=="Elite Scorer" else 3 if arch in ["Playmaker","Scoring Wing"] else -5 if arch=="Rim Anchor" else 1) + high_usage
    wing_r = LEAGUE_ZONE_AVG["Right Wing 3"] + (4 if arch=="Elite Scorer" else 3 if arch in ["Playmaker","Scoring Wing"] else -5 if arch=="Rim Anchor" else 1) + high_usage
    above = LEAGUE_ZONE_AVG["Above Break 3"] + (5 if arch in ["Elite Scorer","Playmaker"] else 3 if arch=="Scoring Wing" else -6 if arch=="Rim Anchor" else 1) + high_usage
